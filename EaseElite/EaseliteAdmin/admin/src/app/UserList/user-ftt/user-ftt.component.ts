...
@Component({
  selector: 'app-user-ftt',
  templateUrl: './user-ftt.component.html',
  styleUrls: ['./user-ftt.component.scss'],
})
export class UserFttComponent implements OnInit {
  ...
  GetAllUserRole(): void {
    this.service.GetAllUserRole().subscribe(
      (data) => {
        ...
        this.filteredUserRoleList = data.filter((user) => user.RoleId === 2);
