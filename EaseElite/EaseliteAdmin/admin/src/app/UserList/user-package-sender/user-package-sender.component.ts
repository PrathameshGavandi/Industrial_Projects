...
@Component({
  selector: 'app-user-package-sender',
  templateUrl: './user-package-sender.component.html',
  styleUrls: ['./user-package-sender.component.scss']
})
export class UserPackageSenderComponent implements OnInit {
  ...
  GetAllUserRole(): void {
    this.service.GetAllUserRole().subscribe(
      (data) => {
        ...
        this.filteredUserRoleList = data.filter((user) => user.RoleId === 5);
