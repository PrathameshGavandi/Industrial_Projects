...
@Component({
  selector: 'app-user-companion',
  templateUrl: './user-companion.component.html',
  styleUrls: ['./user-companion.component.scss']
})
export class UserCompanionComponent implements OnInit {
  ...
  GetAllUserRole(): void {
    this.service.GetAllUserRole().subscribe(
      (data) => {
        ...
        this.filteredUserRoleList = data.filter((user) => user.RoleId === 3);
