...
@Component({
  selector: 'app-carrier-transaction',
  templateUrl: './carrier-transaction.component.html',
  styleUrls: ['./carrier-transaction.component.scss']
})
export class CarrierTransactionComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.GetAllCarrierTransactionDetail(); // Fetch transactions on component init
  }

  // Fetch all transactions
  GetAllCarrierTransactionDetail(): void {
    this.service.GetAllCarrierTransactionDetail().subscribe(
      (data: any[]) => {
        this.ftttransactionList = data;
        console.log('Carrier Transaction Data:', data);
      ...
      error => {
        Swal.fire('Error', 'Failed to fetch Carrier transactions. Please try again later.', 'error');
